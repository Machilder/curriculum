...
public final class ConstSQL {
    ...
    /** パスワード */
    public static final String JDBC_POSTGRES_PASS = "Machilda@7731";

    // FIXME Step-2: 以下のSELECT文、INSERT文、UPDATE文を記述しなさい
    // Tips: 更新値や条件値には「?」を使用してください
    public static final String Select_SQL = "SELECT * FROM Employee WHERE empId = ?";
    
    public static final String Insert_SQL = "INSERT INTO Employee(EmpId,Mail,Password,Name,ProgramingLanguage,Comment) VALUES (?,?,?,?,?,?)  WHERE empId = ?";
    
    public static final String Update_SQL = "UPDATE Employee SET = ? WHERE empId = ?";

    // FIXME Step-2-1: [SQL-SELECT] 社員情報テーブルより、作成日時、更新日時、削除フラグを除いたカラムを取得するクエリを作成しなさい。
    /** 社員情報一覧取得用クエリ: 取得カラム + 取得元テーブル */
    public static final String SELECT_BASE = "SELECT EmpId,Mail,Password,Name,ProgramingLanguage,Comment FROM Employee";
